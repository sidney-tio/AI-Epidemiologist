...
class Agent:
    ...
    def update_state(self, new_state: Enum):
        self.history.append(
            {"time": len(self.history), "old_state": self.state, "new_state": new_state}
        )
...
class SocialSupportSimulation(BaseSimulation):
    def initialize_agents(self):
        for i in range(self.num_agents):
            initial_state = np.random.choice(
                [
                    LonelinessState.CONNECTED,
                    LonelinessState.LONELY,
                    LonelinessState.RECOVERING,
                ],
                p=[0.4, 0.4, 0.2],
            )
            self.agents[i] = Agent(
                agent_id=i, initial_state=initial_state, traits=AgentTraits()
            )
        self.initialize_network()

    def update_agent(self, agent: Agent):
        neighbor_states = agent.get_connection_states(self)
        connected_neighbors = neighbor_states.get(LonelinessState.CONNECTED, 0)
        lonely_neighbors = neighbor_states.get(LonelinessState.LONELY, 0)

        if connected_neighbors > lonely_neighbors:
            agent.update_state(LonelinessState.SUPPORTIVE)
        else:
            agent.update_state(LonelinessState.UNSUPPORTIVE)
...
if __name__ == "__main__":
    ...
    sim = SocialSupportSimulation(num_agents=10, connection_probability=0.4)
