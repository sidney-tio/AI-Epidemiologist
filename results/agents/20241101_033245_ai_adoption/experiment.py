...
class BaseSimulation:
    def __init__(
        ...
        initial_active_ratio: float = 0.1,
        targeted_support: bool = False,
    ):
        self.targeted_support = targeted_support
        self.initial_active_ratio = initial_active_ratio
...
class ExampleSimulation(BaseSimulation):
    def initialize_agents(self):
        for i in range(self.num_agents):
            initial_state = (
                ExampleState.ACTIVE
                if np.random.rand() < self.initial_active_ratio
                else ExampleState.INACTIVE
            )
            self.agents[i] = Agent(
    ...
    def update_agent(self, agent: Agent):
        ...
        if self.targeted_support and agent.state == ExampleState.INACTIVE:
...
if __name__ == "__main__":
    ...
    sim = ExampleSimulation(
        ...
        initial_active_ratio=0.1,
        targeted_support=False,
    )
