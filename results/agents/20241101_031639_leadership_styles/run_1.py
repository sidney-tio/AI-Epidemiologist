...
class Agent:
    def __init__(
        self, agent_id: int, initial_state: Enum, traits: Optional[AgentTraits] = None
    ):
        self.id = agent_id
    ...
    def apply_leadership_style(self, leadership_style: LeadershipStyle, simulation):
        if leadership_style == LeadershipStyle.AUTOCRATIC:
            # Autocratic leadership: force all agents to be ACTIVE
            self.update_state(ExampleState.ACTIVE)
        elif leadership_style == LeadershipStyle.DEMOCRATIC:
            # Democratic leadership: majority rule
            neighbor_states = self.get_connection_states(simulation)
            active_neighbors = neighbor_states.get(ExampleState.ACTIVE, 0)
            if active_neighbors > len(self.connections) / 2:
                self.update_state(ExampleState.ACTIVE)
            else:
                self.update_state(ExampleState.INACTIVE)
        elif leadership_style == LeadershipStyle.LAISSEZ_FAIRE:
            # Laissez-Faire leadership: no change
            pass
        elif leadership_style == LeadershipStyle.TRANSFORMATIONAL:
            # Transformational leadership: inspire change
            self.update_state(ExampleState.ACTIVE)
        elif leadership_style == LeadershipStyle.TRANSACTIONAL:
            # Transactional leadership: reward/punishment
            if self.state == ExampleState.ACTIVE:
                self.update_state(ExampleState.ACTIVE)
            else:
                self.update_state(ExampleState.INACTIVE)
        self.history.append(
...
class BaseSimulation:
    def __init__(self, num_agents: int, connection_probability: float = 0.1):
    ...
    def collect_statistics(self) -> dict:
        stats = {"step": self.step_count, "total_agents": len(self.agents)}

        state_counts = defaultdict(int)
        for agent in self.agents.values():
            state_counts[agent.state] += 1
        stats.update(
            {f"count_{state.value}": count for state, count in state_counts.items()}
        )
...
class ExampleSimulation(BaseSimulation):
    ...
    def update_agent(self, agent: Agent):
        agent.apply_leadership_style(self.leadership_style, self)
...
if __name__ == "__main__":
    ...
    state_columns = [
        col for col in results.columns if col not in ["step", "total_agents"]
    ]
