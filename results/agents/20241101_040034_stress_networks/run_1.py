...
class ExampleSimulation(BaseSimulation):
        ...
        for i in range(self.num_agents):
            self.agents[i] = Agent(
                ...
                traits=AgentTraits(
                    ...
                    resilience=np.random.uniform(0.5, 1.5),
    ...
    def update_agent(self, agent: Agent):
        ...
        # Update stress level based on social support and resilience
        if active_neighbors > len(agent.connections) / 2:
            agent.traits.stress_level -= agent.traits.social_support * 0.1
        else:
            agent.traits.stress_level += (1 - agent.traits.resilience) * 0.1

        # Ensure stress level stays within bounds
        agent.traits.stress_level = max(0.0, min(agent.traits.stress_level, 1.0))

        # Update state based on stress level
        if agent.traits.stress_level < 0.5:
