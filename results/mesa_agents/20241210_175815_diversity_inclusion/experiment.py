...
class ABModel(mesa.Model):
    ...
    def diversity_training(self):
        for agent in self.schedule.agents:
            if agent.gender == "female" or agent.ethnicity != "ethnicity_1":
                agent.wealth += self.diversity_training_effectiveness

    def mentorship_program(self):
        for agent in self.schedule.agents:
            if agent.steps_not_given > 5:
                agent.wealth += 1  # Example effect of mentorship program
        self.schedule = mesa.time.RandomActivation(self)
        self.diversity_training_effectiveness = 0.1  # Example effectiveness rate
...
class Agent(mesa.Agent):
    ...
    def __init__(self, model):
        ...
        self.ethnicity = random.choice(["ethnicity_1", "ethnicity_2", "ethnicity_3"])
